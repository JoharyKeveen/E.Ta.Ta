...
object Catalogue {
    ...
    /** Tous les documents sans rendez-vous : visibles numériquement dans le portefeuille dès le dépôt. */
    val documentsNumeriquesDirects: List<TypeDocument> = certificatsFokontany + autresDocuments

    /** Documents exigeant un passage physique. */
    val documentsSpeciaux = listOf(
        ...
        TypeDocument(
            "arr_cin", "Carte d'identité nationale (CIN)", Guichet.ARRONDISSEMENT,
            surRendezVous = true, pieceOfficiellePermanente = true
        ),
        TypeDocument(
            "arr_permis", "Permis de conduire", Guichet.ARRONDISSEMENT,
            surRendezVous = true, pieceOfficiellePermanente = true
        )
    )
    ...
    fun parId(id: String): TypeDocument? = tous.firstOrNull { it.id == id }
